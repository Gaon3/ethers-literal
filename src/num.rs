...
impl LiteralBaseType {
    const PATTERN: &[char] = &['I', 'U'];
}

impl Display for LiteralBaseType {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Signed => f.write_str("I"),
            Self::Unsigned => f.write_str("U"),
        }
    }
}
...
fn parse_suffix(source: &str) -> Option<(LiteralBaseType, usize, &str)> {
    ...
    // If tpye is unsigned but literal starts with minus
    if base_type == LiteralBaseType::Unsigned && value.starts_with('-') {
        return None;
    }
...
fn transform_literal(source: &str) -> Result<Option<TokenStream>, String> {
    ...
    let Some(mut limbs) = pad_limbs(bits, limbs) else {
...
fn transform_tree(tree: TokenTree) -> TokenTree {
    match tree {
        TokenTree::Group(group) => {
            ...
            let span = group.span();
            let stream = transform_stream_num(group.stream());
